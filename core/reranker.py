...
def rerank(
    ...
    prompt = (
        ...
        f"For each program, write a 'blurb': ONE sentence (max 15 words) about why "
        f"this CAMP matches the query. Use direct, confident, factual language. "
        f"NEVER mention camp type (day/overnight), age range, location/city, province, "
        f"or cost — these are already displayed on the card. "
        f"Focus on: teaching approach, staff credentials, facility quality, "
        f"program structure, or what makes it uniquely relevant. "
